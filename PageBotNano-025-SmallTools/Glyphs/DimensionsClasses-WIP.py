...
class DimensionsTool:
	...
	def drawforeground(self, layer, info):
		...
		y = -250 # Below baseline, could be related to the descender of the glyph
 		for stem, ((x1, xx1), (x2, xx2)) in metrics['stems'].items(): # x1 is smallest-left, x2 is largest-right
			...
			if x1 != xx1 or x2 != xx2:
				label = '(%d)' % int(round(abs(x2 - x1)))
			else:
				label = str(int(round(abs(x2 - x1))))
		...
		x = 0
 		for bar, ((y1, yy1), (y2, yy2)) in metrics['bars'].items():
			path = NSBezierPath.bezierPath()
	...
	def findStems(self, verticals, defaultStem):
		# Looking for stems
		stems = {} # Key (x1, x2), value is vertical
		def1 = defaultStem * self.LO_MARGIN
		def2 = defaultStem * self.HI_MARGIN
		for (x1, xx1) in sorted(verticals.keys()): # Just need to know the x values now
			for (x2, xx2) in sorted(verticals.keys()):
				if x1 <= x2: # Skip mirrors and identical x values
					continue
				if def1 <= abs(x1 - x2) <= def2:
					stem = tuple(sorted((x1, x2))) # Make sure first is the smallest
					stems[stem] = ((x1, xx1), (x2, xx2))
		return stems

	def findBars(self, horizontals, defaultBar):
		# Looking for stems
		bars = {} # Key (x1, x2), value is vertical
		def1 = defaultBar * self.LO_MARGIN
		def2 = defaultBar * self.HI_MARGIN
		for (y1, yy1) in sorted(horizontals.keys()): # Just need to know the x values now
			for (y2, yy2) in sorted(horizontals.keys()):
				if y1 <= y2: # Skip mirrors and identical x values
					continue
				if def1 <= abs(y1 - y2) <= def2:
					bar = tuple(sorted((y1, y2))) # Make sure first is the smallest
					bars[bar] = ((y1, yy1), (y2, yy2))
		return bars
