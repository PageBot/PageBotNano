...
def makePadding(padding, default=0):
    ...
    if isinstance(padding, (list, tuple)):  
        if len(padding) == 2:
            ...
            pt, pr, pb, pl = padding
        else: # In case None or illegal value, then just use default
            raise ValueError('tranformer.padding: Not the right kind of padding "%s"' % padding)
    elif padding is None or isinstance(padding, (int, float, Unit)):
        pt = pr = pb = pl = padding or default
    else: # In case None or illegal value, then just use default
        raise ValueError('transformer.padding: Not the right kind of padding "%s"' % padding)
    if pt is None:
        pt = default
    if pr is None:
        pr = default
    ...
    if pl is None:
        pl = default
    return units(pt), units(pr), units(pb), units(pl)
