...
#   The are combined as class methods, so it is possible to inherit
#   from other template classes, only modifying certain template methods.
#
import sys
...
class OneColumnTemplates(BaseTemplates):
    ...
    def oneColumnPage(cls, theme, doc, page=None, parent=None, pageNumbers=None, **kwargs):
        ...
        >>> theme = BackToTheCity()
        >>> doc = Document()
        >>> page = templates.oneColumnPage(theme, doc, pageNumbers=NONE)
        >>> page.compose(doc)
        >>> page.find(MAIN)
        <TextBox name=mainText w=535 h=782>
        >>> page = doc.newPage(template=templates.oneColumnPage)
        """
        page = cls._initialize(theme, doc, page, parent)
    ...
    def colophonPage(cls, theme, doc, page=None, parent=None, **kwargs):
        ...
        >>> theme = BackToTheCity()
        >>> doc = Document()
        >>> page = templates.colophonPage(theme, doc) # Creating a new page
        >>> page.compose(doc, page)
        >>> page.find(MAIN)
        <TextBox name=mainText w=535 h=782>
        """
        page = cls._initialize(doc, page, parent)
        e = TextBox('', name=MAIN, x=page.pl, y=page.pb, w=page.pw, h=page.ph)
        page.addElement(e)
        return page
